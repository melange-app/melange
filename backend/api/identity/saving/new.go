...
func (i *SaveIdentity) Handle(req *router.Request) framework.View {
	// Decode Body
	...
	_, err = req.Environment.Tables()["identity"].Insert(modelID).Exec(i.Store)
	if err != nil {
	...
	_, err = req.Environment.Tables()["alias"].Insert(modelAlias).Exec(i.Store)
	if err != nil {
	...
	err = req.Environment.Settings.Set("current_identity", id.Address.String())
	if err != nil {
